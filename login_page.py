...
class login_page:
    def __init__(self, root):
        self.window = root
        self.window.title("Log In Gudda's SYSTUMM")
        # Set the window size
    ...
    def login_func(self):
        if self.email_entry.get()=="" or self.password_entry.get()=="":
            ...
            try:
                connection=pymysql.connect(host='localhost', user='root', password='S1911@gudda', database='college2')
                ...
                if row == None:
                    ...
                    messagebox.showinfo("Success","Wellcome to the Gudda's SYSTUMM",parent=self.window)
